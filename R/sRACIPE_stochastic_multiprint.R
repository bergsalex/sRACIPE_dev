...
sRACIPE_stochastic_multiprint <- function( topology_file="inputs/test.tpo", config_file ="inputs/sRACIPE.cfg",ANNEAL=F, NUM_MODELS=100, PARAMETER_RANGE=100, MPR_MIN=1.,MPR_MAX=100, DNR_MIN=0.1,DNR_MAX=1.,FCH_MIN=1,FCH_MAX=100,HCO_MIN=1L,HCO_MAX=6L,STEP_SIZE=0.02,SIM_TIME=50.0,MEDIAN_RANGE=100.0,INITIAL_CONDITIONS=1L,NOISE_LEVELS=30L,MAX_NOISE=30.0,NOISE_SCALING_FACTOR=0.9,SHOT_NOISE_SCALING=0L,GENE_NOISE_SCALING=0L,FILE_WRITING_INTERVAL=1L,OUTPUT_PRECISION=12L, PRINT_START = 50.0, PRINT_INTERVAL=10){
  ...
  if(!exists("configuration")){
    ...
    if(!exists("configuration")) configuration <<- data.frame(ANNEAL=FALSE, NUM_MODELS=1000, PARAMETER_RANGE=100, MPR_MIN=1.,MPR_MAX=100, DNR_MIN=0.1,DNR_MAX=1.,FCH_MIN=1,FCH_MAX=100,HCO_MIN=1L,HCO_MAX=6L,STEP_SIZE=0.02,SIM_TIME=50.0,MEDIAN_RANGE=100.0,INITIAL_CONDITIONS=1L,OUTPUT_PRECISION=12L,THRESHOLD_MODELS=5000,  NOISE_LEVELS=30L,MAX_NOISE=30.0, NOISE_SCALING_FACTOR=0.9, SHOT_NOISE_SCALING=0L,GENE_NOISE_SCALING=0L,FILE_WRITING_INTERVAL=10L,PRINT_START = 50.0, PRINT_INTERVAL=10, stringsAsFactors = T)
  ...
  if(!missing(FILE_WRITING_INTERVAL)){
   ...
   if(!missing(PRINT_START)){
     configuration$PRINT_START <- PRINT_START
   }
   if(!missing(PRINT_INTERVAL)){
     configuration$PRINT_INTERVAL <- PRINT_INTERVAL
