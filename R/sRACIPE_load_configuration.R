sRACIPE_load_configuration <- function(config_file="inputs/sRACIPE.cfg"){
  ...
  configuration <<- data.frame(ANNEALING=FALSE, possible_interactions = 3, NUM_MODELS=1000, PARAMETER_RANGE=100, MPR_MIN=1.,MPR_MAX=100, DNR_MIN=0.1,DNR_MAX=1.,FCH_MIN=1,FCH_MAX=100,HCO_MIN=1L,HCO_MAX=6L,STEP_SIZE=0.02,SIM_TIME=50.0,MEDIAN_RANGE=100.0,INITIAL_CONDITIONS=1L,OUTPUT_PRECISION=12L,THRESHOLD_MODELS=5000, RK_TOLERANCE = 2^(-2), NOISE_LEVELS=2, MAX_NOISE=30, NOISE_SCALING_FACTOR=0.9, standard_deviation_factor=0.5658,SHOT_NOISE_SCALING=0,GENE_NOISE_SCALING=0,FILE_WRITING_INTERVAL=1,CONSTANT_NOISE=1, PRINT_START = 50.0, PRINT_INTERVAL=10, stringsAsFactors = T, PARAMETERS_FILE =0L)
...
configuration$PARAMETERS_FILE <- f_inputs[f_inputs[,1]=="PARAMETERS_FILE",2]
