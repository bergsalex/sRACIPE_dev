...
sRACIPE_RK_deterministic_multiprint <- function(  topology_file="inputs/test.tpo", config_file ="inputs/sRACIPE.cfg", NUM_MODELS=1000, PARAMETER_RANGE=100, MPR_MIN=1.,MPR_MAX=100, DNR_MIN=0.1,DNR_MAX=1.,FCH_MIN=1,FCH_MAX=100,HCO_MIN=1L,HCO_MAX=6L,STEP_SIZE=0.02,SIM_TIME=50.0,MEDIAN_RANGE=100.0,INITIAL_CONDITIONS=1L,OUTPUT_PRECISION=12L,THRESHOLD_MODELS=5000, PRINT_START = 50.0, PRINT_INTERVAL=5){
  ...
  if(missing(config_file)){
    if(!exists("configuration")){
      ...
      if(!exists("configuration")) configuration <- data.frame(NUM_MODELS=1000, PARAMETER_RANGE=100, MPR_MIN=1.,MPR_MAX=100, DNR_MIN=0.1,DNR_MAX=1.,FCH_MIN=1,FCH_MAX=100,HCO_MIN=1L,HCO_MAX=6L,STEP_SIZE=0.02,SIM_TIME=50.0,MEDIAN_RANGE=100.0,INITIAL_CONDITIONS=1L,OUTPUT_PRECISION=12L,THRESHOLD_MODELS=5000,PRINT_START = 50.0, PRINT_INTERVAL=10, stringsAsFactors = T)
      # print(configuration)
    }
  }
  else
  ...
  Time_evolution_test<- multiGeneCircuit_RK_deterministic_multiprint( gene_interaction,  threshold_gene,  configuration$MPR_MIN,  configuration$MPR_MAX, configuration$DNR_MIN,  configuration$DNR_MAX,configuration$possible_interactions,configuration$NUM_MODELS, configuration$THRESHOLD_MODELS,  configuration$STEP_SIZE,  configuration$FCH_MIN,configuration$FCH_MAX, configuration$HCO_MIN,  configuration$HCO_MAX,  configuration$SIM_TIME, configuration$MEDIAN_RANGE, configuration$standard_deviation_factor, topology$number_gene,  configuration$OUTPUT_PRECISION, configuration$INITIAL_CONDITIONS, topology$filename, configuration$PRINT_START, configuration$PRINT_INTERVAL)
...
#  tmp <-   output_file <- paste(working_directory,"/results/sRACIPE_RK_",topology$filename,"_g",toString(topology$number_gene), "_configuration.txt", sep="")

 # write.table(configuration,tmp, quote = FALSE, row.names = FALSE)
