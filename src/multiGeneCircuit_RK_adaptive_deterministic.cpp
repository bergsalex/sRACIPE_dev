...
//extern std::mt19937_64 u_generator;//3 (u_seed);
...
//uniformly distributed random number generator3 in (0,1) range
//extern std::uniform_real_distribution<double> u_distribution;

// Gaussian distributed random number generator3 with mean 0 and 1 standard deviation
...
int multiGeneCircuit_RK_adaptive_deterministic(IntegerMatrix gene_interaction, NumericVector threshold_gene,
                                               ...
                                               int output_precision, int INITIAL_CONDITIONS, double RK_TOLERANCE, String filename)

{
  ...
  std::fstream out_0("./results/sRACIPE_RK_adaptive_"+file_name+"_g"+std::to_string(number_gene)+"_parameters.txt",std::ios::out);
  if(!out_0) {     Rcout << "Cannot open output file for writing parameters.\n";  }

  std::fstream out_ic("./results/sRACIPE_RK_adaptive_"+file_name+"_g"+std::to_string(number_gene)+"_IC.txt",std::ios::out);
  if(!out_ic) {     Rcout << "Cannot open output file for writing parameters.\n";  }

  std::fstream out_1("./results/sRACIPE_RK_adaptive_"+file_name+"_g"+std::to_string(number_gene)+"_output.txt",std::ios::out);
...
//i=i+h;
  ...
  if(h<0.02) Rcout<<h<<"\n";
    ...
    out_1.close();
    out_0.close();
    Rcout<<"Deterministic simulations using Dormand Prince algorithm completed successfully. Data files are in results folder.\n";
    return 0;
}
